...
async function pimpPromptRaw(prompt, model, seed, image = null) {
    ...
    try {
        ...
        if (response.status !== 200) {
            ...
            throw new Error(`Error enhancing prompt: ${response.status} - ${response.statusText}. Body: ${errorBody}`);
        }

        // FIXED: Parse JSON response and extract the enhanced text
        const responseData = await response.json();
        let enhancedPrompt = prompt; // Fallback to original
        ...
        if (responseData.choices && responseData.choices[0] && responseData.choices[0].message) {
            ...
            enhancedPrompt = responseData.choices[0].text;
        } else {
            // If we can't parse the JSON structure, try to use the full response as text
            const responseText = JSON.stringify(responseData);
            logError("Unexpected API response format:", responseText.substring(0, 200) + "...");
        }
